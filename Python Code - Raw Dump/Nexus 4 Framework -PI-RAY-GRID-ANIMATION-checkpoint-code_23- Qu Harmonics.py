...
dark_energy = 0.1  # dark energy influence
dark_matter = 0.3  # dark matter influence
...
c = np.zeros(len(t))  # damping coefficient
...
for n in range(len(t) - 1):
    ...
    # Mary’s Spirit: Smoothly adjust c towards c_target with dark influences
    c_adjust = c_target * (1 + dark_energy * np.exp(-zeta_n) - dark_matter * np.exp(-t[n]))
    c[n + 1] = c[n] * (1 + np.exp(-kappa * (zeta_n - zeta_target)))
    if abs(zeta_n - zeta_target) < 0.01:
        c[n + 1] = c_adjust
    
    # Equations of motion with dark effects
    a = (-k * x[n] - c[n] * v[n]) / m + dark_energy * x[n] - dark_matter * v[n]
    v[n + 1] = v[n] + a * dt
    x[n + 1] = x[n] + v[n] * dt
...
plt.title("Nexus-Stabilized Oscillator with Dark Effects")
plt.xlabel("Time (s)")
