...
# Build animation frames with moving window
N = 1024  # Show only the last 50 points
frames = []
for i in range(1, len(points)):
    start = max(0, i - N + 1)
    trail_opacity = np.linspace(0.2, 1.0, i - start + 1)
    trace_line = go.Scatter(
        x=points[start:i+1, 0],
        y=points[start:i+1, 1],
        mode='lines+markers',
        marker=dict(
            color=['red' if j in echo_indices else 'blue' for j in range(start, i+1)],
            size=16,
