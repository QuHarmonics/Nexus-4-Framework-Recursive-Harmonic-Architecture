...
if not is_equal:
    ...
    print("Indices where data mismatch occurs:", np.where(binary_data != retrieved_data)[0])
    print("Original values at mismatched indices:", binary_data[np.where(binary_data != retrieved_data)[0]])
    print("Decoded values at mismatched indices:", retrieved_data[np.where(binary_data != retrieved_data)[0]])
