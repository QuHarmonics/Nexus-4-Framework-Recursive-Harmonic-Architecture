...
plt.subplot(3, 1, 1)
plt.plot(binary_to_waveform(np.array([int(b) for b in hash_to_binary(input_string)])), label="Initial Binary Waveform", color='blue')
...
plt.subplot(3, 1, 2)
plt.plot(hashed_waveform, label="Hashed Binary Waveform", color='orange', linestyle='-')
...
plt.subplot(3, 1, 3)
plt.plot(filtered_waveform, label="Filtered Waveform (After Zeta Zeros Applied)", color='green', linestyle='--')
