...
def fold_pi_digits(chunk, mode="position_product", factor=2):
    if mode == "mirror_sum":
        ...
        resonance = sum(sums) / 36  # (4 pairs, each max 9+9)
    elif mode == "position_product":
        positions = list(range(2, 10))  # Always 2..9 for every chunk
        products = [d * p for d, p in zip(chunk, positions)]
        resonance = sum(products) / 63  # classic: (max per digit 9*9=81, 8 values, or just sum(positions))
    elif mode == "frequency":
        ...
        resonance = sum(gaps) / 16  # (max gap 8*2)
    else:
...
for idx, chunk in enumerate(chunks):
    ...
    for mode in ["mirror_sum", "position_product", "frequency"]:
        resonance = fold_pi_digits(chunk, mode=mode)
        tag = "Aligns with RHA harmonic range (0.30-0.40)" if 0.30 <= resonance <= 0.40 else "Outside harmonic range, exploring further folds"
