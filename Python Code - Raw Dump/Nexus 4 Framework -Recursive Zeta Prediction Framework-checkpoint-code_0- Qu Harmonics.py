...
# Recursive Zeta Prediction Framework
...
# Operand Selection with Phase Tracking
def modulating_operand_selection(delta_b, phase):
    """
    Select operands based on recursive feedback and phase dynamics.
    """
    if phase % 4 == 1:
        return "addition"
    elif phase % 4 == 2:
...
# Combine Zeta and Cosine Effects with Phase Tracking
def apply_zeta_cos_effect(formula_result, zeta_ratio, cos_value, phase):
    """
    Combine zeta and cosine effects into a final result.
    - Zeta flips the result periodically.
    - Cosine provides smooth modulation.
    - Phase adds an evolving cyclic component.
    """
    zeta_flip = -1 if zeta_ratio > 0.5 else 1  # Zeta provides push-pull dynamics
    cos_modulation = math.cos(phase * math.pi / 180)  # Smooth modulation over phase
    return formula_result * zeta_flip * cos_modulation

# Calculate Next Digit with Recursive Feedback
def calculate_next_digit(current_digit, previous_digit, bit_space, phase, zeta_ratio):
    """
    Compute the next π digit using recursive relationships with zeta and cosine dynamics.
    """
    delta_b = current_digit - previous_digit
    cos_value = math.cos(current_digit)

    # Operand selection based on phase dynamics
    operation = modulating_operand_selection(delta_b, phase)
    ...
    # Combine zeta and cosine effects
    final_result = apply_zeta_cos_effect(formula_result, zeta_ratio, cos_value, phase)

    # Compute the next digit
    next_digit = round(final_result % 10)

    # Update formula length (bit space)
    formula = f"{operation}: ({current_digit} with {delta_b}) - {harmonic_correction} + {residual_correction} (Phase: {phase})"
    new_bit_space = len(formula)

    return next_digit, formula, new_bit_space

# Generate π Sequence with Phase Evolution
def generate_pi_sequence(initial_digit, length):
    """
    ...
    for step in range(1, length):
        ...
        next_digit, formula, new_bit_space = calculate_next_digit(current_digit, previous_digit, bit_space, phase, zeta_ratio)
...
# Example Usage
initial_digit = 3
sequence_length = 500# Generate a longer sequence to observe patterns
pi_sequence, formulas_used = generate_pi_sequence(initial_digit, sequence_length)
